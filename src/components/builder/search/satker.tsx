...
const SatkerSearchBuilder = ({ satkerList, id }: SatkerSearchBuilderProps) => {
			...
			<PopoverTrigger asChild>
				...
				>
					<span
						className={cn(
							"truncate",
							search.has(id) && search.get(id) !== null
								? "opacity-100"
								: "opacity-70",
						)}
					>
						{search.has(id) && search.get(id) !== null
							? satkerList.find(
									(item) =>
										decodeId(item.id) === decodeId(search.get(id) ?? ""),
								)?.nama
							: "Select Satker..."}
					</span>
								...
								>
									<Check
										className={cn(
											"mr-2 h-4 w-4",
											search.has(id) &&
												decodeId(search.get(id) ?? "") === decodeId(item.id)
												? "opacity-100"
												: "opacity-0",
										)}
									/>
